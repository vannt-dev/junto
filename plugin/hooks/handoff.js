...
// src-hooks/handoff.ts
function handleHandoff(input) {
  if (input.tool_name !== "Agent" && input.tool_name !== "Task") return "";
  const root = findProjectRoot(input.cwd ?? process.cwd());
  if (root === null) return "";
  const id = readActiveId(root);
  if (id === null) return "";
  const path = join3(taskDir(root, id), "context.jsonl");
  if (!existsSync3(path)) return "";
  const rows = readFileSync2(path, "utf-8").split("\n").map((line) => line.trim()).filter((line) => line !== "").map((line) => {
    try {
      return JSON.parse(line);
    } catch {
      return {};
    }
  }).filter((row) => row._example === void 0 && typeof row.file === "string");
...
if (process.argv[1]?.replace(/\\/g, "/").endsWith("/handoff.js")) {
  await runHook((input) => handleHandoff(input));
}
export {
  handleHandoff
};
