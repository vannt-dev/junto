...
var PROTECTED_GLOBS = [
  ...
  ".junto/tasks/*/contexts/**",
  // Generated by junto from repository facts; the model must not edit what the reviewer is told.
  ".junto/tasks/*/plan.resolved.json",
  ".junto/tasks/*/review-background.md",
  ".junto/tasks/*/review-events.jsonl",
  ".junto/tasks/*/review-report.html"
];
...
function readConfig(root) {
  const path = join2(juntoDir(root), "config.json");
  if (!existsSync2(path)) {
    throw new Error(
...
function readActiveId(root) {
  const path = join2(juntoDir(root), "active");
  if (!existsSync2(path)) return null;
  const id = readFileSync(path, "utf-8").trim();
  return id === "" ? null : id;
}
function readTask(root, id) {
  const path = join2(taskDir(root, id), "task.json");
...
if (process.argv[1]?.replace(/\\/g, "/").endsWith("/guard.js")) {
  await runHook((input) => handleGuard(input));
}
export {
  handleGuard,
  isProtected
